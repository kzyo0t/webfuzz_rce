...
from .types         import FuzzerLogger, get_logger, HTTPMethod, RequestStatus, Statistics, ExitCode, UnimplementedHttpMethod, InvalidContentType, InvalidHttpCode, RCEConfidence
...
# we are logged in        logger.error("Aborting due to lack of fuzz targets")

LOGGED_IN_CHECK_INTERVAL = 50
debug_logger = get_logger("debug_kzy_worker","")#kzy_code
...
class Worker():
    ...
    def update_stats(self, current_node: Node):
        ...
        self._stats.total_rce = self._detector.rce_count
    ...
    async def process_req(self, request: Node) -> RequestStatus:
        ...
        async with self.http_send(request) as r:
            #debug_logger.info(r.status)
            raw_html: str = await r.text()
            #kzy_code
            raw_statuscode: str = r.status
            ...
            #if self._detector.check_time(request.exec_time):
            #    self._detector.xss_scanner(request, next(soup))


            #kzy_code
            if self._detector.rce_precheck(raw_html,raw_statuscode):
                self._detector.rce_scanner(request,raw_html, raw_statuscode)
            ...
            self._crawler += links
            #debug_logger.info(len(cfg))
            #debug_logger.info("%s",cfg)
            ...
            if (request._rce_confidence > RCEConfidence['NONE']):
