...
HEADS = 1
TAILS = 2
debug_logger = get_logger("debug_kzy_mutate","")#kzy_code
...
class Mutator:
    def __init__(self):
        ...
        self.xss_payloads: Payloads = Payloads([
            ...
            Kind(weight=20, payloads=read_tokens("Payloads/os_rce/well_formed"))
        ])
    ...
    def mutate(self, from_node: Node, node_list: List[Node]) -> Node:
        ...
        if from_node.size == 0:
            ...
            #debug_logger.info("choices %s",choice)
            new_params = choice(from_node, node_list)
            #debug_logger.info("choice1 %s",new_params)
        ...
        debug_logger.info("Mutated node %s",new_node)
        return new_node
    ...
    def add_strrce_payload(self,
                           param:str,
                           val: List[str]) -> Tuple[str, List[str]]:
        logger = logging.getLogger(__name__)
        logger.debug("Mutate fun insert random str+rce")

        (param2,val2) = self.add_rce_payload(param, val)

        return Mutator.add_random_text(param2, val2)
    def add_rce_payload(self, 
                        param: str, 
                        val: List[str]) -> Tuple[str, List[str]]:
        """
            Prepends or appends a random xss payload
